	...
	vir_bytes buf, size_t size, endpoint_t for_e)
{
  ...
  f->filp_pos = position;
	...
	struct vmnt *vpath;
	vpath = find_vmnt(vp->v_fs_e);
	if(strcmp(vpath->m_mount_path, "/home") == 0){

		if(rw_flag == READING){
			printf("MINIX 210010060 : file read: %llu; nbytes =%u; offset = %llu\n",vp->v_inode_nr,size,f->filp_pos);	
		}
		else if(rw_flag == WRITING){
			printf("MINIX 210010060 : file write: %llu; nbytes =%u; offset = %llu\n",vp->v_inode_nr,size,f->filp_pos);	
		}
	} 
