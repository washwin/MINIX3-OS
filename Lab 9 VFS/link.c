 ...
 */
  ...
  if ((dirp->v_mode & S_ISVTX) == S_ISVTX) {
	...
	stickycheck.l_vmnt_lock = VMNT_READ; // set these locks to read; other processes shouldn't get
	stickycheck.l_vnode_lock = VNODE_READ;
	vp = advance(dirp, &stickycheck, fp);// get vnode object of file (resolving path to node)
  // (advance dir ptr to next file, and return vnode of file to be unlinked)
