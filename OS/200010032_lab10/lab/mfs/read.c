...
static void wipe_inode(struct inode *rip); // function declaration
/*===========================================================================*
 *				fs_readwrite				     *
 *===========================================================================*/
int fs_readwrite(void)
{
  ...
  struct inode *rip;
  size_t nrbytes;
  int immediate=0; // took immediate variable
  r = OK;
  ...
  // regular = (mode_word == I_REGULAR || mode_word == I_NAMED_PIPE); // commented this
  regular = (mode_word == I_REGULAR || mode_word == I_NAMED_PIPE ||mode_word==I_IMMEDIATE);// added immediate type
  ...
  if(mode_word==I_IMMEDIATE) // for immediate files
  {
	 ...
	 {
	   if(position+nrbytes>32) // checking if file size becomes > 32 after the content added if yes then the filetype is chaged to regular
	   {
		     char* temp;
		     char inode_data[40];
	     ...
	     rip->i_mode = (I_REGULAR | (rip->i_mode & ALL_MODES)); // setting mode type to regular
	     mode_word = rip->i_mode & I_TYPE;
...
// this function is got from mfs/inode.c
/*===========================================================================*
 *       wipe_inode             *
 *===========================================================================*/
